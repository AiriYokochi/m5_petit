...
static const unsigned long SILENCE_TIMEOUT_MS = 2000;
...
#define ROUTER_IP_LAST 102
#define MDNS_HOSTNAME "puchiru"
#define WG_CLIENT_PRIVATE_KEY WG_PUCHIRU_PRIVATE_KEY
#define WG_CLIENT_IP          WG_PUCHIRU_IP
...
void setup() {
  ...
  if (wifiConnected) {
    ...
    // mDNS: http://puchiru.local/ でアクセス可能に
    if (MDNS.begin(MDNS_HOSTNAME)) {
      MDNS.addService("http", "tcp", 80);
      MDNS.addService("ws", "tcp", 8080);
      Serial.printf("mDNS: %s.local\n", MDNS_HOSTNAME);
