...
#define ROUTER_IP_LAST 101
#define MDNS_HOSTNAME "puchiteya"
#define WG_CLIENT_PRIVATE_KEY WG_PUCHITEYA_PRIVATE_KEY
#define WG_CLIENT_IP          WG_PUCHITEYA_IP
...
// ===== タッチ反応 =====
volatile bool touchColorFlash = false;
unsigned long touchColorFlashStart = 0;
const unsigned long TOUCH_FLASH_DURATION = 400;
uint16_t baseFaceColor = TFT_LIGHTGREY;  // setup()で上書き
uint16_t warmFaceColor = TFT_LIGHTGREY;  // setup()で上書き

// ===== 最終タッチ時刻 =====
unsigned long lastTouchEventTime = 0;  // 起動からのms
...
void checkTouch() {
  ...
  if (touchPhase == TOUCH_IDLE) {
    if (touch.isPressed()) {
      ...
      // 顔色フラッシュ（あたたかい色に一瞬）
      if (!touchColorFlash) {
        touchColorFlash = true;
        touchColorFlashStart = millis();
        currentFaceColor = warmFaceColor;
      }
    }
  } else {
    if (!touch.isPressed()) {
...
void drawSettingsScreen() {
  ...
  if (getLocalTime(&t, 10)) {
    ...
    faceSprite.setCursor(6, 9); faceSprite.print(buf);
  } else {
    faceSprite.setCursor(6, 9); faceSprite.print("--:--  NTP syncing");
  }
  ...
  faceSprite.setTextSize(2); faceSprite.setCursor(6, 54);  faceSprite.printf("%3d%%", BRIGHTNESS_STEPS[brightnessIdx]);
...
void drawMenuOverlay() {
  ...
  faceSprite.setTextSize(3); faceSprite.setCursor( 22,  28); faceSprite.print("CAM");
  faceSprite.setTextSize(1); faceSprite.setCursor( 22,  82); faceSprite.print("Camera");
...
void wakeUp() {
  ...
  // 2. 目をゆっくり開ける（blinkを手動制御）
  for (int step = 0; step <= 20; step++) {
    // BLINK_DURATION=250ms のアニメーションを逆再生するイメージ
    // step 0=閉じ → step 20=開き
    float openness = step / 20.0f;
    // blinkの三角波を逆にして目の開きを制御
    blinking = (openness < 1.0f);
    if (blinking) {
      // blinkStartTimeを調整して半開き状態を作る
      // p=0.5(全閉)→p=1.0(全開) の後半を使う
      float p = 0.5f + openness * 0.5f;
      blinkStartTime = millis() - (unsigned long)(p * BLINK_DURATION);
    }
    // 視線もゆっくり上がる
    float lookY = 20.0f * (1.0f - openness);
    drawFace(0, (int)lookY, 0);
    delay(40);
  }
  ...
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      if (strcmp(p, "END") == 0) {
        ...
        baseFaceColor = currentFaceColor;
...
void sendSensorPacket() {
  ...
  json += "\"last_touch\":" + String(lastTouchEventTime);
  json += "}";
...
void setup() {
  ...
  baseFaceColor = currentFaceColor;
  warmFaceColor = colorFromHex("ffb347");  // あたたかいオレンジ寄り
  ...
  if (wifiConnected) {
    ...
    // mDNS: http://puchiteya.local/ でアクセス可能に
    if (MDNS.begin(MDNS_HOSTNAME)) {
      MDNS.addService("http", "tcp", 80);
      MDNS.addService("ws", "tcp", 8080);
      Serial.printf("mDNS: %s.local\n", MDNS_HOSTNAME);
...
void loop() {
  ...
  if (blinking && (now - blinkStartTime) >= BLINK_DURATION) {
    blinking = false;
    nextBlinkTime = now + random(3000, 8000);  // ぷちてゃはゆっくり
  }

  // タッチ色フラッシュの復帰
  if (touchColorFlash && (now - touchColorFlashStart) >= TOUCH_FLASH_DURATION) {
    touchColorFlash = false;
    currentFaceColor = baseFaceColor;
  }
  ...
  if (requestPlaySound) {
    ...
    // タッチ音(pon.wav)は小さめに再生
    bool isTouchSound = (pendingSoundName == "pon.wav");
    uint8_t savedVol = currentVolumeRaw;
    if (isTouchSound) {
      currentVolumeRaw = max((uint8_t)1, (uint8_t)(savedVol / 4));
    }
    playWavFromSD(("/wav/" + pendingSoundName).c_str());
    if (isTouchSound) {
      currentVolumeRaw = savedVol;
    }
  }
